...
const IMAGE_CARD_CLASS = 'mt-4 w-full max-w-xs rounded-[28px] border border-orange-100 bg-[#fff8f1] p-3 shadow-[0_16px_35px_rgba(209,110,52,0.16)] sm:max-w-sm';
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
      <div className="py-4">
        <Link
          to="/dashboard"
          className="text-md cursor-pointer text-orange-600 hover:text-orange-700 dark:text-orange-300 dark:hover:text-orange-400"
        >
          ← Back to Dashboard
        </Link>
      </div>
          ...
          {entries.length > 0 ? (
            <>
              {paginatedEntries.map((entry) => (
                <div
                  key={entry.id}
                  className="w-full rounded-2xl border border-pink-300 bg-gradient-to-br from-white/100 to-white/50 p-4 shadow-lg backdrop-blur-md dark:bg-white sm:p-6"
                >
                  <li className="flex flex-col gap-5 sm:gap-6 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0 flex-1">
                      <h1 className="text-2xl font-extrabold text-black sm:text-3xl">{entry.title}</h1>
                      <h6 className="mt-1 text-xs font-bold text-black">
                        {new Date(entry.updatedAt).toLocaleString('en-US', dateOptions)}
                      </h6>
                      <label className="mt-4 block font-bold text-black">
                        Mood :
                        <span className="ml-1 inline-block text-md font-semibold italic text-orange-700">
                          {entry.mood}
                        </span>
                      </label>
                      <div
                        className="archive-entry-content mt-3 text-balance break-words text-black"
                      ...
                      {entry.imageUrl ? (
                        <div className={IMAGE_CARD_CLASS}>
                          <div className="aspect-[4/5] w-full overflow-hidden rounded-[20px] bg-white shadow-inner">
                            <img
                              src={entry.imageUrl}
                              alt={entry.title}
                          ...
                          <p className="mt-3 text-center text-xs font-semibold uppercase tracking-[0.18em] text-orange-700/70">
                            Attached memory
                          </p>
                        </div>
                      ) : null}
                      {entry.aiReply ? (
                        <label className="mt-5 block font-bold text-black">
                          Reflection :
                          <div
                            className="mt-2 break-words text-orange-700"
                    ...
                    <div className="flex flex-wrap items-center gap-2 lg:w-auto lg:flex-col lg:items-end">
                      <button
                        ...
                        disabled={isDisabled}
                        className="rounded-lg p-1 transition hover:bg-orange-100"
                      >
              ...
              {entries.length > ENTRIES_PER_PAGE ? (
                <div className="mt-2 flex flex-col items-center gap-3 rounded-2xl border border-orange-200/70 bg-white/60 px-4 py-4 shadow-sm sm:flex-row sm:justify-between">
                  <p className="text-sm font-medium text-orange-900/75">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="rounded-full border border-orange-200 bg-white px-4 py-2 text-sm font-medium text-orange-800 transition hover:bg-orange-50 disabled:cursor-not-allowed disabled:opacity-45"
                    >
                      Previous
                    </button>
                    <div className="rounded-full bg-orange-100/80 px-4 py-2 text-sm font-semibold text-orange-900">
